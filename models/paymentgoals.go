...
func (db *DB) GetPaymentGoals() ([]*PaymentGoal, error) {

	sql := `
	SELECT * FROM payment_goals WHERE aggregate = True LIMIT 1
	`
	...
	goals := make([]*PaymentGoal, 0)
	for rows.Next() {
		goal := new(PaymentGoal)
		err := rows.Scan(&goal.Id, &goal.Aggregate, &goal.PaymentTypeId, &goal.GoalDate, &goal.Amount)
		if err != nil {
			return nil, err
		}
